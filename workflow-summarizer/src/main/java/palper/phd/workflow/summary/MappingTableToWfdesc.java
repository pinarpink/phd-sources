	...
	public static InfModel decoratePortsWithMappings(InfModel model,
		...
		for (Resource res : processors) {
			...
			for (Resource inp : inpPorts) {
				...
				String origiNalUri = traceToOriginal(cloneUri,
						clonetoOriginalPorts);
				if (origiNalUri == null) {
					continue;
				} else {
...
//			
//			
//			Set<Resource> wfInport = WfDescRdfUtils.getInputPorts(WfDescRdfUtils.getWorkflowResource(model), model);
//			for (Resource inp : wfInport) {
//				String cloneUri = inp.getURI();
//				String origiNalUri = traceToOriginal(cloneUri,
//						clonetoOriginalPorts);
//				if (origiNalUri == null) {
...
//			
//			
//			Set<Resource> wfOutport = WfDescRdfUtils.getOutputPorts(WfDescRdfUtils.getWorkflowResource(model), model);
//			
	...
	private static void decorateModel(String originalPortUri,
			String clonePortUri, InfModel model) {

		model.add(
				model.getResource(clonePortUri),
				model.createProperty(SummarizerNamespaces.motifsNS + "cloneOf"),
				originalPortUri);
